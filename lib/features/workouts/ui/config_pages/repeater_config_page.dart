  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: CustomScrollView(
        ...
        slivers: [
          // 1. The Header (You can reuse your GenericWorkoutHeader or build a custom one)
          SliverToBoxAdapter(
            child: Padding(
                ...
                children: [
                  GestureDetector(
                    ...
                    child: Row(
                      ...
                      children: [
                        Icon(Icons.arrow_back_ios_rounded, size: 14, color: accentColor.withOpacity(0.8)),
                        const SizedBox(width: 4),
                        Text('Workouts', style: context.cardTitle.copyWith(fontSize: 13, color: accentColor.withOpacity(0.8))),
                      ],
                    ),
                  ),
                  const SizedBox(height: 24),
                  Text("REPEATERS", style: context.h1.copyWith(fontSize: 28)),
                  const SizedBox(height: 2),
                  Text("Timed hang sets with rest intervals", style: TextStyle(fontSize: 13, color: context.textMuted)),
                ],
              ),
            ),
          ),
              ...
              delegate: SliverChildListDelegate([
                ...
                AppCard(
                  ...
                  child: SegmentedControl(
                    ...
                    accentColor: accentColor,
                    onChanged: (idx) => notifier.updateStartingHand(Hand.values[idx]),
                  ),
                ),
                const SizedBox(height: 24), // Extra spacing before the intensity group
                    ...
                    children: [
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          DropdownControl(
                            choices: const ['Bodyweight', 'Max Lift', 'Custom'],
                            selectedIndex: config.relativeTo,
                            accentColor: accentColor,
                            onChanged: (val) => notifier.updateRelativeTo(val),
                          ),
                        ],
                      ),
                      ...
                      Padding(
                        ...
                        child: Divider(color: context.cardBorder, height: 1),
                      ),
                            ...
                            ? Column(
                                children: [
                                  ...
                                  Row(
                                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                    children: [
                                      Text("Custom Weight", style: context.body),
                                      WeightInput(
                                        weightKg: config.customWeight,
                                        accentColor: accentColor,
                                        onChangedKg: notifier.updateCustomWeight,
                                      ),
                                    ],
                                  ),
                                ],
          ...
          SliverToBoxAdapter(
            child: Padding(
              padding: EdgeInsets.fromLTRB(16, 24, 16, MediaQuery.of(context).padding.bottom + 40),
              child: GestureDetector(
                onTap: () {
                  HapticFeedback.mediumImpact();
                  
                  // 1. Generate the final state
                  final activeState = notifier.buildActiveState();
                  
                  // 2. Navigate and override the config provider for the engine
                  Navigator.of(context, rootNavigator: true).push(
                    CupertinoPageRoute(
                      builder: (_) => ProviderScope(
                        overrides: [
                           // Pass the cleanly generated state to the live engine!
                           repeaterConfigProvider.overrideWithValue(activeState),
                        ],
                ...
                child: Container(
                  ...
                  child: Center(
                    child: Text(
                      'Start Repeaters',
                      style: context.cardTitle.copyWith(fontSize: 15, fontWeight: FontWeight.w800, color: context.background),
                    ),
                  ),
                ),
              ),
